 ...
 *
 * Updated:
 * Fix the function ft_MDblClk trying to emulate the ft_MButPrs function that works only on DOS platform.
 * this allows to run also in x64 windows console shell
 */
...
FUNCTION ft_MDblClk( nClick, nButton, nInterval, nRow, nCol, nStart )
   ...
   LOCAL lstButton     // status of last button pressed
   LOCAL nButtPrs      // last button that had a click
...
#else     
      nButtPrs := ft_MGetCoord( @nVert, @nHorz )
      if nButtPrs != lstButton
      	if nButtPrs >0
      		lstButton := nButtPrs
      		nRow := nVert
      		nCol := nHorz
